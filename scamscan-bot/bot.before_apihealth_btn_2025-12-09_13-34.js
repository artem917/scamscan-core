...
async function formatApiHealthMessage() {
  try {
    ...
    const rpcNetworks = (h && h.rpc && Array.isArray(h.rpc.networks))
      ? h.rpc.networks.join(", ")
      : "n/a";
...
bot.command('admin', async (ctx) => {
  if (!ensureAdmin(ctx)) return;
  await ctx.reply(getAdminHelpText(), { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[ { text: 'API Health', callback_data: 'ADMIN_HEALTH' } ]] } });
});
